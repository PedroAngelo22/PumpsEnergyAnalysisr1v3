...
# Mapeia nomes de materiais para suas rugosidades absolutas (em mm)
MATERIAIS = {
...
# Mapeia nomes de acessórios para seus coeficientes de perda de carga (K)
K_FACTORS = {
...
def calcular_perdas_trecho(trecho, vazao_m3h, fluido_selecionado):
    ...
    # Busca a rugosidade a partir do material selecionado
    ...
    # Calcula a perda localizada com base nos acessórios adicionados
...
# (O restante das funções de cálculo e geração de gráfico/diagrama não precisa de alterações)
...
# --- Interface do Aplicativo (Atualizada) ---
...
# Função auxiliar para renderizar um trecho na UI
def render_trecho_ui(trecho, prefixo, lista_trechos):
    ...
    c1.selectbox("Selecionar Acessório", options=list(K_FACTORS.keys()), key=f"selectbox_acessorio_{trecho['id']}")
    c2.number_input("Qtd", min_value=1, value=1, step=1, key=f"quantidade_acessorio_{trecho['id']}")
...
# (A lógica principal não precisa de alterações)
try:
    ...
    if distribuicao_vazao:
        ...
        for i, (nome_ramal, vazao_ramal) in enumerate(distribuicao_vazao.items()):
            trecho_ref = st.session_state.ramais_paralelos[nome_ramal][0]; velocidade = calcular_perdas_trecho(trecho_ref, vazao_ramal, fluido_selecionado)['velocidade']
